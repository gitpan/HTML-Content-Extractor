...
=item * my $ratio = new HTML::WordTagRatio::RelativeRatio()
...
$value is computed as follows:
	...
	($WordCount[$end] - $WordCount[$start])/(($WordCount[$end] - $WordCount[$start]) + ($TagCount[$end] - $TagCount[$start]))
...
package HTML::WordTagRatio::RelativeRatio;
use strict;
use warnings;
use Carp;
...
# new - constructs RelativeRatio object
# - preconditions: 	none
# - postconditions: 	RelativeRatio is constructed
sub new
{
	my $invocant = shift;	
...
sub RangeValue
{
	...
	my $NinRange = $N[$j] - $N[$i];
	my $TinRange = $T[$j] - $T[$i];
	return $NinRange/($NinRange + $TinRange);
}
1;
...
=head1 SYNOPSIS
  ...
  my $ratio = new HTML::WordTagRatio::NormalizedRatio();
...
HTML::WordTagRatio::NormalizedRatio computes a ratio of Words to Tags for a given range. In psuedo code, the ratio is 

Words/(TotalWords + Tags)

=head2 Methods

=over 4

=item * my $ratio = new HTML::WordTagRatio::NormalizedRatio()

Initializes HTML::WordTagRatio::NormalizedRatio

=item * my $value = $ratio->RangeValue($start, $end, \@WordCount, \@TagCount)

$value is computed as follows:
	
	($WordCount[$end] - $WordCount[$start])/(($WordCount[$#WordCount]) + ($TagCount[$end] - $TagCount[$start]))
...
ContentExtractorDriver.pl (1), HTML::Content::ContentExtractor (3), HTML::Content::HTMLTokenizer (3), HTML::WordTagRatio::Ratio (3),HTML::WordTagRatio::WeightedRatio (3), HTML::WordTagRatio::SmoothedRatio (3), HTML::WordTagRatio::RelativeRatio (3), HTML::WordTagRatio::ExponentialRatio (3).

=cut


package HTML::WordTagRatio::NormalizedRatio;
use strict;
use warnings;
use Carp;
...
# new - constructs NormalizedRatio object
# - preconditions: 	none
# - postconditions: 	NormalizedRatio is constructed
sub new
{
	my $invocant = shift;	
